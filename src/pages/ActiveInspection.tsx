...
export default function ActiveInspection() {
  ...
  useEffect(() => {
    if (itemCount > prevItemCount.current) {
      ...
      if (navigator.vibrate) {
        if (hasFail) {
          navigator.vibrate([100, 50, 100, 50, 100]); // triple buzz for fail
        } else if (hasMonitor) {
          navigator.vibrate([100, 50, 100]); // double buzz for monitor
        } else {
          navigator.vibrate(50); // single tap for pass/normal
        }
      }
  ...
  // Coverage gap haptic: buzz when stopping with gaps
  const handleStop = useCallback(async () => {
    const coverage = itemCount / totalFields;
    if (coverage < 0.7 && navigator.vibrate) {
      navigator.vibrate([200, 100, 200, 100, 200]); // warning: significant gaps
    }
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-card border-b border-border">
        <div className="flex items-center gap-2">
          <div className={`w-2.5 h-2.5 rounded-full ${scribe.isConnected ? 'bg-status-fail animate-recording-pulse' : 'bg-muted-foreground'}`} />
          <span className={`text-[10px] font-mono font-semibold ${scribe.isConnected ? 'text-status-fail' : 'text-muted-foreground'}`}>
            {scribe.isConnected ? 'REC' : isConnecting ? 'CONNECTING' : 'OFF'}
          </span>
          <span className="text-[10px] font-mono text-muted-foreground ml-2">{formatTime(elapsed)}</span>
        </div>
        <div className="flex items-center gap-3">
          {isAnalyzing && (
            <div className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-sensor animate-pulse-glow" />
              <span className="text-[9px] font-mono text-sensor">ANALYZING</span>
            </div>
          )}
          <span className="text-[10px] font-mono text-muted-foreground">{machine.assetId}</span>
        </div>
      </header>

      {/* Camera preview */}
      {isCameraOn && (
        <div className="relative mx-4 mt-3 rounded overflow-hidden border border-border" style={{ height: '100px' }}>
          <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover" />
          <div className="absolute top-1.5 left-1.5 flex items-center gap-1 bg-background/80 px-1.5 py-0.5 rounded text-[9px] font-mono text-foreground">
      ...
      <div className="p-4 bg-card border-t border-border">
        <button
          onClick={handleStop}
          className="w-full flex items-center justify-center gap-2 py-3.5 rounded bg-status-fail text-accent-foreground font-bold text-sm transition-all hover:brightness-110"
        >
